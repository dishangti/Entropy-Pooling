clear all
close all
load  'CaseStudy_A300.mat';

b_grid = [-1:0.5:5];
min_X= min(A300_path(1:tau_-1));
Delta_X = (mu_LT-min_X)/2;
X_grid = [min_X:Delta_X:min_X+Delta_X*10];
max_X= max(A300_path(1:tau_-1));
Delta_X = (-mu_view+max_X)/2;
X_grid_viewMean = sort([max_X:-Delta_X*2:max_X-Delta_X*30]);
...
ylabel(h1,'Exposure','fontsize',10,'FontWeight', 'bold', 'Units','pixels');     
line(t(1:tau_-1),b_NoMI_prior,'Color','k','Parent',ax1_h1);
...
ax2_h1 = axes('Position',get(ax1_h1,'Position'),...
           ...
           'Color','none',...
           'XColor','k','YColor','r');
line(t(1:tau_-1), repmat(mu_LT,1,tau_-1),'Color','r','Linestyle','--','Parent',ax2_h1);
line(t(1:tau_-1),A300_path(1:tau_-1),'Color','r','Parent',ax2_h1);       
...
ylabel(ax2_h1,'A300','fontsize',10,'FontWeight', 'bold', 'Units','pixels');     
...
line(t(1:tau_-1),b_NoMI_post,'Color','k','Parent',ax1_h2);
line(t(1:tau_-1),b_MI_Bellman_post(1:end-1,1),'Color','b','Parent',ax1_h2);
...
ylabel(h2,'Exposure','fontsize',10,'FontWeight', 'bold', 'Units','pixels');     
...
ylabel(h3,'Exposure','fontsize',10,'FontWeight', 'bold', 'Units','pixels');     
xlabel('t(years)')
...
ax2_h3 = axes('Position',get(ax1_h3,'Position'),...
           ...
           'Color','none',...
           'XColor','k','YColor','r');
line(t(1:tau_-1), repmat(mu_LT,1,tau_-1),'Color','r','Linestyle','--','Parent',ax2_h3);
line(t(1:tau_-1),A300_path(1:tau_-1),'Color','r','Parent',ax2_h3);       
...
ylabel(ax2_h3,'A300','fontsize',10,'FontWeight', 'bold', 'Units','pixels');     
...
ylabel(h4,'Exposure','fontsize',10,'FontWeight', 'bold', 'Units','pixels');     
line(t(1:tau_-1),b_NoMI_viewMean,'Color','k','Parent',ax1_h4);
...
ax2_h4 = axes('Position',get(ax1_h4,'Position'),...
           ...
           'Color','none',...
           'XColor','k','YColor','r');
line(t(1:tau_-1), repmat(mu_view,1,tau_-1),'Color','r','Linestyle','--','Parent',ax2_h4);
line(t(1:tau_-1),A300_path(1:tau_-1),'Color','r','Parent',ax2_h4);       
...
ylabel(ax2_h4,'A300','fontsize',10,'FontWeight', 'bold', 'Units','pixels');     
